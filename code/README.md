...
```
- **설명**
  - YOLO ONNX 모델, Intel RealSense D435, USB 웹캠, SQLite DB, Arduino 시리얼 통신을 초기화합니다.
...
```
- **설명**
  - 도로, 횡단보도, 신호등 영역을 ROI(Region of Interest) 좌표로 직접 설정하였습니다.
  - 이후 YOLO로 검출된 사람 및 차량 객체와 ROI의 겹침 비율을 계산하여 무단횡단, 차량 침범, 불법 유턴 등의 이벤트를 위치 기반으로 판정하였습니다.
...
```
- **설명**
  - SQLite DB를 활용하여 이벤트 로그와 일별 통계를 저장하는 구조를 설계하였습니다.
  - event_logs 테이블에는 이벤트 종류, 발생 시각, 이미지 경로를 저장하고,
  - daily_stats 테이블에는 날짜별 무단횡단 횟수와 마지막 감지 시간을 관리하였습니다.
  - 이를 통해 실시간 감지뿐 아니라 사후 로그 조회 및 통계 분석이 가능하도록 구현하였습니다.
  - 이벤트 발생 시 DB 저장과 동시에 이미지 파일 경로를 함께 기록하여, 추후 이벤트 이미지 조회 기능과 연동되도록 구성하였습니다.
---
...
def judge_warning_light(roi):
    ...
    BRIGHTNESS_THRESHOLD = 180  # 밝기 고려 (단순 빨간 물체가 아니라 실제로 켜진 경광등인지 확인(오탐 방지 로직))
...
# 신호등 색깔 판정 함수 (위치가 고정이기 때문에 좌표 고정으로 판단)
...
- **설명**
  - OpenCV의 HSV 색 공간 변환과 마스크 연산을 활용하여 신호등과 경광등의 실제 점등 상태를 판별하였습니다.
  - YOLO 객체 검출 결과만 사용하는 것이 아니라, ROI 내부의 색상 비율을 추가로 분석하여 빨간불, 초록불, 경광등 ON 상태를 구분하도록 구현하였습니다.
  - 이를 통해 단순 객체 검출을 넘어 상태 기반 이벤트 판단이 가능하도록 설계하였습니다.
...
```
- **설명**
  - 사람 객체가 치량 신호 초록불에 횡단보도에 검출되면 횡단보도 ROI와의 겹침 비율을  계산하였고 신호와 상관없이 횡단보도가 아닌 도로에 침범시 도로 ROI와의 겹침비율을 계산하였습니다.
  - 이후 겹침 비율이 기준값 이상인 경우를 무단횡단 상황으로 판정하도록 구현하였습니다.
  - 이를 통해 단순히 사람을 검출하는 것이 아니라, 객체의 위치 정보를 바탕으로 실제 도로 위반 상황을 해석하도록 설계하였습니다.
---
...
- **설명**
 - **설명**
  - Intel RealSense D435의 깊이 정보를 활용하여 사람 객체 중심점의 실제 거리를 계산하였습니다.
  - 중심점의 depth 값을 구한 뒤, 2D 픽셀 좌표를 3D 공간 좌표로 변환하고, 횡단보도 기준선 샘플 점들과의 유클리드 거리를 비교하여 최소 거리를 산출하였습니다.
  - 이를 통해 무단횡단 상황을 단순 검출이 아닌 실제 거리(cm) 정보와 함께 표시할 수 있도록 구현하였습니다.
---
...
```
- **설명**
  - 차량 객체가 차량신호 빨간불일 경우에 횡단보도 ROI와의 겹침비율을 계산하였습니다.
  - 차량 객체가 검출되면 좌측 차로, 우측 차로 ROI와의 겹침 비율을 각각 계산하였습니다.
  - 이를 기반으로 차량 침범, 불법 유턴, 불법주차를 구분하여 판정하도록 구현하였습니다.
  - 특히 불법주차는 특정 구역에 일정 시간 이상 머무는 경우로 정의하여 단순 위치 검출을 넘어 시간 기반 이벤트 판단이 가능하도록 설계하였습니다.
...
```
- **설명**
  - fall 클래스가 검출될 경우와 경광등 ON을 감지할 경우 비상상황으로 판정합니다.
  - 경광등 ON 감지 시 비상상황 전환 및 차량신호 3개를 모두 점등하여 비상상황임을 인지시킬 수 있게 합니다.
